...
type oldConfiguration struct {
	...
	TypeMappings       map[string]string            `yaml:"type-mappings"`
}
...
func updateConfigFromFlags(cfg configuration) (configuration, error) {
	...
	if flagTypeMappings != "" {
		unsupportedFlags = append(unsupportedFlags, "--type-mappings")
	}
...
func updateOldConfigFromFlags(cfg oldConfiguration) oldConfiguration {
	...
	if cfg.TypeMappings == nil && flagTypeMappings != "" {
		var err error
		cfg.TypeMappings, err = util.ParseCommandlineMap(flagTypeMappings)
		if err != nil {
			errExit("error parsing type-mappings: %s\n", err)
		}
	}
	return cfg
}
...
func newConfigFromOldConfig(c oldConfiguration) configuration {
	...
	opts.OutputOptions.TypeMappings = cfg.TypeMappings
